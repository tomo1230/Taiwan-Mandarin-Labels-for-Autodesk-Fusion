...
對照表是以英文文字為鍵的純 JSON，單筆修改可直接編輯檔案：

```json
{ "Extrude": "您偏好的術語" }
```

若要大量修改，`csv_tools.py` 可將兩份對照表透過 CSV 來回轉換，
以便在試算表中編輯：

```bash
python csv_tools.py export     # -> zh_tw.csv, zh_tw_long.csv
python csv_tools.py import     # -> 寫回 .json
python csv_tools.py check      # 回報譯文遺漏的符號
```

開始之前有三點需要知道：

- 只編輯 `traditional` 欄。`english` 是用來與 Fusion 標籤比對的鍵，
  改動它會讓該條目失效
- 英文側帶有介面實際會用到的控制字元——`&` 快速鍵、結尾的 `...`、
  `%1%` 佔位符、標記與 `href` 目標。拿掉其中任何一個，
  改變的是行為而不只是措辭
- **Excel 會破壞 79 個條目**，這些條目以 `+`、`-` 或 `=` 開頭，
  會被判讀為公式。請以匯入方式開啟，不要雙擊檔案

每次寫入前都會先在 `backups/` 建立帶時間戳記的備份；
若匯入會刪除超過 10% 的條目，則直接拒絕。

**完整指南：[EDITING.zh-TW.md](EDITING.zh-TW.md)**——符號對照、Excel 匯入步驟、
復原方式，以及 `import` 如何處理不完整的資料列。

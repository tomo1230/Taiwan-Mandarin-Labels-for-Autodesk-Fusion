...
| `href="..."` | 說明連結的目標 | 連結失效 |
| `(...)` | 可能是快速鍵容器、狀態（`(All)`）或複數（`Row(s)`） | 視情況而定，移除前請先判讀 |
...
它會回報佔位符、標記、連結目標、快速鍵、結尾 `...` 與多餘空白，並附上範例：
...
Autodesk 自己的簡體中文資料本身就有 90 處這類不一致，因此數字不為零是正常的。
請在編輯前後各執行一次並比較差異，而不是以歸零為目標。

### 備份

`build_dict.py` 與 `csv_tools.py` 在覆寫之前，都會先將目前的對照表複製到
`backups/`，並以當下時間命名：
...
這一點很重要，因為以 `build_dict.py` 重建會從 StringTable 重新產生兩份對照表，
並捨棄所有手動修正。每份對照表保留最近 20 個世代，更舊的會自動清除。

要復原編輯內容，只需將該檔案複製回 `zh_tw.json`——由於檔名依時間排序，
最新的世代就是字母順序中的最後一個。
